...
def lab2c(robot,qseed,ee_link,ee_local_position,ee_local_axis,target,target_axis):
    ...
    lp = ee_local_position
    la = ee_local_axis
    ee = robot.link(ee_link)
    robot.setConfig(qseed)
    posgoal = ik.objective(ee, local=lp, world=target)
    #axgoal = ik.objective(ee, R=target_axis, t = lp)
    ...
    ee_world_position = ee.getWorldPosition(lp)
    ee_world_direction = ee.getWorldDirection(la)
